...
{  
    ...
    if (command == "PWM")
    {
      ...
      Serial.println(data);
      // digitalWrite(PIN_PWM, data); // FIXED: Wrong function - digitalWrite only sets HIGH/LOW, not PWM
    ...
    if (command == "BRAKE")
    {
      ...
      Serial.println(data);
      // analogWrite(PIN_BRAKE, data); // FIXED: Wrong function - brake should be digital on/off
      // digitalWrite(PIN_BRAKE, data); // FIXED: Need to invert for active LOW brake
      digitalWrite(PIN_BRAKE, !data); // Brake is active LOW - invert the command
    }
    ...
    if (command == "DIR")
    {
      ...
      Serial.println(data);
      // analogWrite(PIN_DIR, data); // FIXED: Wrong function - direction should be digital forward/reverse
